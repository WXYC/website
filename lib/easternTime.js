...
/**
 * Same shape as {@link CALENDAR_DATE} but without the weekday, hoisted for
 * call sites that format many dates per render rather than one per page —
 * `formatPlayDate` in `lib/flowsheetSearch.js` runs this once per
 * search-result row from a non-memoized row component, where a fresh
 * `Intl.DateTimeFormat` per call measured ~0.06ms against ~0.0005ms reused.
 */
const CALENDAR_DATE_NO_WEEKDAY = new Intl.DateTimeFormat('en-US', {
	timeZone: 'UTC',
	month: 'long',
	day: 'numeric',
	year: 'numeric',
...
/**
 * A calendar date as a human-readable Eastern date, without the weekday.
 *
 * Prefer this over `formatCalendarDate(dateStr, {weekday: undefined})` in any
 * call site that formats many dates per render rather than one per page —
 * see the note on {@link CALENDAR_DATE_NO_WEEKDAY}, the formatter this
 * reuses instead of constructing on every call.
 *
 * @param {string} dateStr Calendar date, `YYYY-MM-DD`.
 * @returns {string} e.g. `July 21, 2026`.
 */
export function formatCalendarDateNoWeekday(dateStr) {
	const [y, m, d] = dateStr.split('-').map(Number)
	const noon = new Date(Date.UTC(y, m - 1, d, 12))
	return CALENDAR_DATE_NO_WEEKDAY.format(noon)
}
...
/**
 * An instant as a numeric Eastern date, `MM/DD/YYYY`.
 *
 * Station wall clock, not the reader's: a play logged at 11:06 PM in Chapel
 * Hill is already the next day in UTC, and dating it by anything but the
 * Eastern calendar files it under a day WXYC did not broadcast it. Built on
 * {@link easternDateOf} rather than a fresh `Intl` formatter so there is one
 * place that decides what day an instant belongs to, and so the zero-padding
 * comes from the same `2-digit` parts the rest of this module uses — a column
 * of dates that shifts width row to row is harder to read than the two extra
 * characters cost.
 *
 * Year in full rather than tubafrenzy's and dj-site's two-digit `M/D/YY`. Not
 * because those surfaces show one day at a time — dj-site's Previous Sets
 * renders `M/D/YY` over an archive it advertises as "November 2004 to present"
 * — but because a two-digit year is ambiguous over a span that long, and this
 * column is read down rather than glanced at. The two characters are cheap.
 *
 * @param {string} isoTimestamp ISO 8601 timestamp.
 * @returns {string} e.g. `08/05/2026`, or the empty string if unparseable.
 */
export function formatNumericDate(isoTimestamp) {
	const date = easternDateOf(isoTimestamp)
	if (date === null) return ''
