...
import {formatEasternTime, formatNumericDate} from './easternTime'
...
/**
 * A `play_date` timestamp, as an Eastern date and time.
 *
 * Numeric, matching the live playlist's date column, because those two pages
 * are one journey now that the playlist's search box hands off to this one —
 * and the same play shown on both should not be spelled two ways. It was
 * `July 21, 2026, 11:47 AM` while they were separately-reached pages.
 *
 * The time stays. It is the difference between the two columns and it is not
 * cosmetic: the playlist dates a row whose set header and neighbours already
 * establish the hour, whereas a search result arrives with no context at all,
 * so "when" has to mean the instant rather than the day.
 *
 * Both halves come from `lib/easternTime.js` rather than a second
 * date-formatting path: {@link formatNumericDate} resolves the Eastern
 * calendar day the instant falls on (not the UTC day, which would misfile
 * anything within a few hours of midnight) and pads it to a fixed width.
 *
 * @param {?string} playDate
 * @returns {string} e.g. `07/21/2026, 11:47 AM`, or the empty string if
 *   `playDate` is missing or unparsable.
 */
export function formatPlayDate(playDate) {
	if (!playDate) return ''
	const normalized = normalizePgTimestamp(playDate)
	const date = formatNumericDate(normalized)
