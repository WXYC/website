 ...
 *
 * Four entry types carry no `message` at all. `dj_join` / `dj_leave` and
 * `show_start` / `show_end` all name a DJ in `dj_name` instead, so the
 * `message`-then-type-name fallback at the bottom would print the bare wire
 * token — "show start", "dj join" — for every one of them. Backend's V2
 * projection is explicit about this: its `show_start`/`show_end` arm emits
 * `dj_name` and a `timestamp` and nothing else, and its `dj_join`/`dj_leave`
 * arm emits `dj_name` alone (`flowsheet.service.ts` `transformToV2`).
 *
 * The sign-on and sign-off wording is the station's own, carried over from
 * tubafrenzy's recent-entries page ("START OF SHOW: DJ Starcross'd SIGNED ON
 * at 6:58 PM (6/4/25)") minus the shouting, and matching dj-site's Classic
 * flowsheet ("Start of show — {dj_name} @ {date} {time}"). The clock time is
 * kept because these rows span the whole table: they are the one row type that
 * cannot carry a date cell of its own, so dropping the time would leave the
 * only rows that say a show changed hands as the only rows that never say
 * when. The date is not repeated here — the surrounding rows carry that.
 *
 * A row whose `add_time` is missing or unparseable says so by omission rather
 * than by printing "at " and stopping.
 *
 * A handle that did not resolve arrives here as the empty string, not null:
 * the same projection defaults it with `entry.dj_name ?? ''`. Both fall
 * through to the unattributed wording, which says the show changed hands
 * without inventing a name for who was holding it.
 *
 * @param {{entry_type: string, message: ?string, dj_name: ?string, add_time: ?string}} entry
 * @returns {string}
 */
export function describeNonTrackEntry(entry) {
	if (entry.entry_type === 'show_start' || entry.entry_type === 'show_end') {
		const verb = entry.entry_type === 'show_start' ? 'signed on' : 'signed off'
		const who = entry.dj_name || null
		const line = who ? `${who} ${verb}` : verb[0].toUpperCase() + verb.slice(1)
		const at = formatEasternTime(entry.add_time)
		return at ? `${line} at ${at}` : line
	}
...
/**
 * Order a flat, multi-show entry list most-recent-first: latest show first,
 * and within a show by the DJ's stated `play_order` rather than by the
 * `add_time`/insertion order the endpoint returns. This is the descending,
 * cross-show counterpart to the `play_order` sort {@link groupRangeByDay}
 * applies within a single already-grouped show — same rule (see that
 * function's docstring for the production numbers behind it), applied here
 * to a flat list that also has to establish show-to-show order for itself.
 *
 * The `id` tie-break exists because `play_order` collisions are real, not a
 * data-quality bug to paper over: the tubafrenzy webhook and the dj-site
 * live-insert path assign it independently, and there is no per-show UNIQUE
 * constraint. Backend's own `getEntriesByShow` breaks that same tie on
 * `desc(id)`, which this mirrors. Leaving the tie-break out and relying on
 * `Array.prototype.sort`'s stability would silently fall back to arrival
 * order for collided rows — exactly what a `play_order` sort exists to
 * override.
 *
 * `show_id` and `play_order` are coalesced to `0` before subtracting rather
 * than compared directly, so a `show_id: null` row (20 of 2,619,011 rows in
 * production — see {@link groupRangeByDay}) sorts as a declared, defined
 * number instead of leaning on `null`'s implicit coercion in arithmetic. This
 * choice of `0` is not display-neutral: every real `show_id` is a positive
 * integer, so in this descending sort a coalesced `0` is the lowest possible
 * value and an unattributed row sorts *below every attributed row*, however
 * recently it aired — a show that went to air minutes ago still displays
 * above an unattributed row from hours earlier. Unlike {@link groupRangeByDay},
 * which buckets these rows into a labelled "Unattributed" block, this flat,
 * ungrouped list (consumed by the live playlist page, `pages/playlist.jsx`)
 * has no equivalent marker — the row just appears at the bottom with no
 * annotation explaining why. At ~20 of 2,619,011 rows this is rare enough
 * that the placement is arguably fine as-is; noted here so a future reader
 * doesn't mistake the coalescing for merely NaN-avoidance with no visible
 * consequence.
 *
 * @param {{show_id: ?number, play_order: number, id: number}} a
 * @param {{show_id: ?number, play_order: number, id: number}} b
 * @returns {number} Negative if `a` sorts before `b`, positive if after.
 */
export function compareEntriesByAirOrderDesc(a, b) {
	return (
		(b.show_id ?? 0) - (a.show_id ?? 0) ||
		(b.play_order ?? 0) - (a.play_order ?? 0) ||
		(b.id ?? 0) - (a.id ?? 0)
	)
}
