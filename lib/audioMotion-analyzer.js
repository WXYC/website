...
/**
 * LOCAL MODIFICATIONS (WXYC)
 * Original: https://github.com/hvianna/audioMotion-analyzer v4.5.2
 *
 * Changes made:
 * 1. Added `fixedBarWidth` option - overrides calculated bar width with exact pixel value
 * 2. When `fixedBarWidth` is set, `barSpace` is treated as pixel gap between bars
 * 3. Modified bar rendering to draw from vertical center (bars grow up and down)
 * 4. Added `setCustomData(values)` / `clearCustomData()` / `hasCustomData()` methods
 *    to override frequency data with custom values (for initial state and CORS fallback)
 */
...
const DEFAULT_SETTINGS = {
	...
	ansiBands      : false,
	barSpace       : 0.1,
	fixedBarWidth  : 0,  // when > 0, overrides calculated bar width (in pixels)
	bgAlpha        : 0.7,
 ...
 */
	constructor( container, options = {} ) {
		...
		*/
		...
		// Finish canvas setup
		this._ready = true;
		this._customBarValues = null; // For overriding bar values with custom data
	...
	set fixedBarWidth( value ) {
		this._fixedBarWidth = +value || 0;
		this._calcBars();
	}
	...
	/**
	 * Sets custom bar values to override real audio data.
	 * Values should be normalized in [0, 1] range.
	 * Pass an array of values (one per bar) or a single value for all bars.
	 *
	 * @param {array|number} values - Array of values or single value
	 */
	setCustomData( values ) {
		if ( typeof values === 'number' ) {
			// Single value for all bars
			this._customBarValues = Array( this._bars.length ).fill( values );
		} else if ( Array.isArray( values ) ) {
			this._customBarValues = values;
		}
	}

	/**
	 * Clears custom bar values, returning to real audio data.
	 */
	clearCustomData() {
		this._customBarValues = null;
	}

	/**
	 * Returns whether custom data is currently set.
	 * @returns {boolean}
	 */
	hasCustomData() {
		return this._customBarValues !== null;
	}
	...
	_calcBars() {
		...
		*/
		...
		// When fixedBarWidth is set, use fixed pixel dimensions for bars and gaps
		if ( this._fixedBarWidth > 0 ) {
			const fixedWidth = this._fixedBarWidth;
			// When barSpace >= 1, treat as pixel gap; otherwise use as ratio of bar width
			const fixedGap = _barSpace >= 1 ? _barSpace : _barSpace * fixedWidth;
			const totalBarWidth = fixedWidth + fixedGap;

			// Start from the left edge and go right
			bars.forEach( ( bar, index ) => {
				const posX = initialX + index * totalBarWidth;
				bar.posX = posX;
				bar.barCenter = posX + fixedWidth / 2;
...
*/
			...
			for ( let barIndex = 0; barIndex < nBars; barIndex++ ) {
				...
				// Use custom data if available, otherwise compute from FFT
				if ( this._customBarValues && barIndex < this._customBarValues.length ) {
					barValue = Math.max( 0, Math.min( 1, this._customBarValues[ barIndex ] ) );
				} else {
					barValue = Math.max( interpolate( binLo, ratioLo ), interpolate( binHi, ratioHi ) );
				...
				if ( _mode == MODE_GRAPH ) {
					...
					// When fixedBarWidth is set, check if bar is at least partially visible
					// Otherwise use original check (posX >= initialX)
					else if ( this._fixedBarWidth > 0 ? ( posX + width >= initialX && posX < initialX + analyzerWidth ) : posX >= initialX ) {
						if ( _radial )
							radialPoly( posX, 0, width, barHeight, isOutline );
						else if ( isRound ) {
							// MODIFIED: Draw round bars centered vertically with rounded caps
							const halfWidth = width / 2;
							const centerY = ( channelTop + analyzerBottom ) / 2;
							// Minimum bar height ensures a dot (circle) when barHeight is 0
							const effectiveHeight = Math.max( barHeight, 0 );
							const halfBar = effectiveHeight / 2;

							_ctx.beginPath();
							// Draw a rounded rectangle (stadium shape) centered vertically
							// Top cap (semicircle)
							_ctx.arc( barCenter, centerY - halfBar, halfWidth, PI, 0, false );
							// Right side down to bottom cap
							_ctx.arc( barCenter, centerY + halfBar, halfWidth, 0, PI, false );
							_ctx.closePath();
							strokeIf( isOutline );
							_ctx.fill();
						}
							...
							_ctx.beginPath();
							// MODIFIED: Draw bars centered vertically (grow from middle both up and down)
							const centerY = ( channelTop + analyzerBottom ) / 2;
							const halfBar = ( barHeight + offset ) / 2;
							_ctx.rect( posX, centerY - halfBar, width, barHeight + offset );
							strokeIf( isOutline );
							_ctx.fill();
						}
					}
